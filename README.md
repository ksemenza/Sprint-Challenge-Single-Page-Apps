...
With client-side routing, the whole page doesn’t have to be re-rendered. Only the elements that change, due to the changed state of the application, are re-rendered.
Transitions and animation are easier to implement. Ultimately the code creates a lot more versatility without as much code and integration of smaller separate files.
